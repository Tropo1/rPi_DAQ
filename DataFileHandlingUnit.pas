...
var
 f:File;
begin
//ConsoleWindowWriteLn(WindowHandle,'writing... ');
Assign(f,fname);
if FileExists(fname) then
 begin
...
{procedure SaveSampleToFile2(WindowHandle:TWindowHandle;f:File;SampleData:TSampleArray);
var
 f:File;
begin
ConsoleWindowWriteLn(WindowHandle,'writing... ');
...
procedure HandleFile(sensor,signal:String;NewFileRate:LongWord; DT_now:TDateTime; LeadString:String; var f:File; var DT_file:TDateTime; var fname:String);
 // DT_now           : StartTime from current SampleBuffer in TDateTime type
 // DT_last          : Time from when the last file created in TDateTime type
 // NewFileRate      : Time in minutes after which a new file should be created
...
begin
 ...
 //DecodeDate(DT_now,y_now,m_now,d_now);
 //DecodeDate(DT_last,y_last,m_last,d_last);
